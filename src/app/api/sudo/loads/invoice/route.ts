// src/app/api/sudo/loads/invoice/route.ts
...
export async function GET(req: Request) {
  try {
    ...
    // cache key for Invoice‐relevant fields
    const key = hash({
    ...
    const folderFS  = path.join(process.cwd(), "public", "uploads", "LRInvoice", "sudo", "Invoice");
    const filename  = `Invoice-${load.loadNumber ?? id}-${key}.pdf`;
    const fileFS    = path.join(folderFS, filename);
    const filePublic = `/uploads/LRInvoice/sudo/Invoice/${filename}`;
