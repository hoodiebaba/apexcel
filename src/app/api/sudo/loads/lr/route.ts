// src/app/api/sudo/loads/lr/route.ts
...
export async function GET(req: Request) {
  try {
    ...
    // cache key for LR‐relevant fields
    const key = hash({
    ...
    const folderFS  = path.join(process.cwd(), "public", "uploads", "LRInvoice", "sudo", "LR");
    const filename  = `LR-${load.loadNumber ?? id}-${key}.pdf`;
    const fileFS    = path.join(folderFS, filename);
    const filePublic = `/uploads/LRInvoice/sudo/LR/${filename}`;
    const absUrl = new URL(filePublic, req.url).toString();

    // if cached file exists → redirect
    ...
    // generate minimal white PDF with data
    await renderPdfToFile((doc) => {
      const line = (label: string, val: any) => {
        doc.fontSize(11).text(`${label}: ${val ?? "-"}`);
      };

      doc.fontSize(16).text("Lorry Receipt", { align: "left" });
      doc.moveDown(0.5);
