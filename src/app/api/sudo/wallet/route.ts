...
async function getMe() {
  // NOTE: yahan same sudo/admin cookie verify ho raha hai.
  // Agar vendor/customer ko bhi login dena hai, unke tokens bhi yahin parse karo.
...
export async function GET(req: Request) {
  ...
  if (op === "members") {
    ...
    // vendor/customer → sirf sudo/admin lists dekh sakte
  ...
  if (op === "file") {
    ...
    // access: sudo OR (sender/receiver)
    const row = await prisma.wallet.findUnique({ where: { id } });
    if (!row) return NextResponse.json({ error: "Not found" }, { status: 404 });
    if (
      me.role !== "sudo" &&
      !(row.requestById === me.id || row.requestToId === me.id)
    ) {
    ...
    // Proof store path: /public/uploads/wallet/{senderRole}/{senderUsername}/{storedName}
    ...
    if (mode === "view" && isPreviewable(row.paymentProof)) {
      ...
      headers["Content-Disposition"] = `attachment; filename="${(row.utrNumber || "proof").replace(/"/g, "")}${path.extname(row.paymentProof)}"`;
      return new NextResponse(ab, { headers });
    }
  }
...
export async function POST(req: Request) {
  ...
  if (op === "delete") {
    ...
    for (const r of rows) {
      // sudo → hard delete (and proof unlink), else → mark failed + append remark
      if (me.role === "sudo") {
        if (r.paymentProof) {
