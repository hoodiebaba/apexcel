...
async function getMe() {
  ...
  try {
    ...
    if (!admin || admin.status !== "active") return null; // status gate
    return {
      id: admin.id,
      role: admin.role,
      name: admin.name ?? "",
...
// API side: pure admin ko full access, warna perms check
...
// Safe file writer (extension guess)
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    // tag KYC by on status change
    if (typeof data.kycStatus === "string" && data.kycStatus.length > 0) {
      data.kycBy = who(me);
    }

    // password hashing if provided non-empty
    ...
    // auto customerName if blank but first/last present
    ...
    console.error("ADMIN_CUSTOMER_UPDATE_ERROR", e);
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    console.error("ADMIN_CUSTOMER_DELETE_ERROR", e);
