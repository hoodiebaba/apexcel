...
function CustomerModal({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (meRes.status === 401) return onClose();
      const payload = await meRes.json();
      const m = (payload?.user || payload) as Me;
      setMe(m);

      if (isCreate) {
        setForm({ registeredBy: roleName(m), kycStatus: "pending" });
      } else if (selectedId) {
        const r = await fetch(`/api/admin/customer?id=${selectedId}`, { credentials: "include", cache: "no-store" });
  ...
  async function save() {
    ...
    const r = await fetch("/api/admin/customer", { method, body: fd, credentials: "include" });
    if (!r.ok) {
  ...
  function openDoc(type: "pan" | "gst" | "aadhar" | "cheque") {
    if (!form.id) return;
    const url = `/api/admin/customer/file?id=${form.id}&type=${type}`;
...
export default function CustomerPage() {
  ...
  // sirf ADMIN allow (sudo/customer block). Buttons perms-based.
  ...
  async function refresh() {
    const url = new URL(location.origin + "/api/admin/customer");
  ...
  useEffect(() => {
    (async () => {
      ...
      const payload = await meRes.json();
      const m: Me = (payload?.user || payload) as any;

      if (m?.status && m.status !== "active") { router.replace("/admin/login"); return; }
      if (m?.role !== "admin") { router.replace("/admin"); return; } // only admin

      setMe(m);
      const ok = has(m?.permissions, "customer:page_view");
    ...
    <div className="p-6">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <button onClick={openCreate} disabled={!can("customer:create")}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm ${can("customer:create")
        ...
        <button onClick={openEdit} disabled={!can("customer:edit") || selectedIds.length !== 1}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm ${can("customer:edit") && selectedIds.length === 1
            ? "border-gray-300 text-gray-700 bg-white hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
            : "opacity-50 cursor-not-allowed border-gray-300 dark:border-gray-700"}`}>
          <SquarePen size={16} /> Edit
        </button>
        <button onClick={openView} disabled={!can("customer:view") || selectedIds.length !== 1}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm ${can("customer:view") && selectedIds.length === 1
            ? "border-gray-300 text-gray-700 bg-white hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
            : "opacity-50 cursor-not-allowed border-gray-300 dark:border-gray-700"}`}>
          <BadgeInfo size={16} /> View
        </button>
        <button onClick={doDelete} disabled={!can("customer:delete") || selectedIds.length === 0}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm ${can("customer:delete") && selectedIds.length > 0
            ? "border-gray-300 text-gray-700 bg-white hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
            : "opacity-50 cursor-not-allowed border-gray-300 dark:border-gray-700"}`}>
              ...
              {rows.map((c) => (
                <tr key={c.id} className="border-t dark:border-gray-700">
                  <td className="px-5 py-3">
                    <input type="checkbox" checked={selectedIds.includes(c.id)} onChange={() => setSelectedIds((p)=> p.includes(c.id) ? p.filter(x=>x!==c.id) : [...p, c.id])}/>
                  </td>
