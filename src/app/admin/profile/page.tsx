...
export default function AdminProfilePage() {
  ...
  const sanitizeData = (data: Record<string, any>) => {
    ...
    for (const [k, v] of Object.entries(data)) {
      ...
      if (k === "password" && String(v).trim() === "") continue;
      cleaned[k] = v;
    }
    delete cleaned.role;
  ...
  const handleSave = async () => {
    try {
      ...
      if (file) {
        ...
        Object.entries(cleanData).forEach(([k, v]) => form.append(k, String(v)));
  ...
  if (loading) {
    return <div className="w-full p-6 text-center text-gray-500 dark:text-gray-400">Loading profile…</div>;
  }

  if (forbidden) {
    return (
      <div className="max-w-2xl mx-auto p-8">
        <h2 className="text-xl font-semibold mb-2">No access to Profile</h2>
        <p className="text-sm text-gray-600">Aapke account me is page ka access nahi hai.</p>
      </div>
    );
  }
