...
const API_BASE = "/api/sudo/wallet"; // <-- single source of truth
...
function CreateModal({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`${API_BASE}?op=members&role=${rolePick}`);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-xl relative">
        <button className="absolute top-3 right-3 text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white" onClick={onClose} disabled={busy}>
          <X size={18} />
        </button>
...
function ViewModal({ open, row, onClose }: { open: boolean; row: WalletRow | null; onClose: () => void; }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-lg relative">
        ...
        <div className="p-5 space-y-2 text-sm">
          ...
          {row.paymentProof && (
            <div className="pt-2">
              <a href={`${API_BASE}?op=file&id=${row.id}`} target="_blank" className="inline-flex items-center gap-2 text-blue-601 dark:text-blue-400">
                <Download size={16} /> Download Proof
              </a>{" "}
              <a href={`${API_BASE}?op=file&id=${row.id}&mode=view`} target="_blank" className="inline-flex items-center gap-2 text-blue-600 dark:text-blue-400">
                Preview
              </a>
            </div>
          )}
        ...
        <div className="p-4 border-t dark:border-gray-700 text-right">
          <button className="px-4 py-2 rounded-lg text-sm border dark:border-gray-700" onClick={onClose}>Close</button>
        </div>
      </div>
    </div>
  );
...
function EditModal({ open, row, onClose, onUpdated }: { open: boolean; row: WalletRow | null; onClose: () => void; onUpdated: () => void; }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md relative">
        <button className="absolute top-3 right-3 text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white" onClick={onClose} disabled={busy}>
          <X size={18} />
        </button>
        <div className="p-5 border-b dark:border-gray-700">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="text-xs text-gray-500 mt-1">History format: <code>role:username:remark</code>.</div>
          </div>
        </div>
        <div className="p-4 border-t dark:border-gray-700 text-right">
          <button className="px-4 py-2 rounded-lg text-sm border dark:border-gray-700 mr-2" onClick={onClose} disabled={busy}>Cancel</button>
...
export default function WalletPage() {
  ...
  async function fetchMeThenData() {
    try {
      const mres = await fetch("/api/sudo/loads/fetch?type=me");
      const m = (await mres.json()) as Me | null;
      setMe(m);

      if (!m) { window.location.href = "/sudo"; return; }
      const ok = m.role === "sudo" || (m.permissions || []).includes("wallet:page_view");
  ...
  async function doDelete() {
    ...
    const sure = confirm(me?.role === "sudo" ? "Hard delete selected entries?" : "Mark as failed and append deletion remark?");
    if (!sure) return;
  ...
  const badge = (status: WalletRow["status"]) =>
    status === "success"
      ...
      <div className="flex items-center justify-start mb-6 gap-3">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-300 text-sm font-medium ${
              btn.enabled
                ? "text-gray-700 bg-white hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
        ...
        <div className="max-w-full overflow-x-auto">
          <div className="min-w-[1100px]">
            <table className="w-full text-sm table-auto">
              ...
              <tbody>
                {rows.map((entry) => (
                  <tr key={entry.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 border-t dark:border-gray-700">
                    ...
                    <td className="px-5 py-3 flex items-center gap-2 text-gray-700 dark:text-gray-300">
                      ...
                      {entry.paymentProof && (
                        <>
                          <a href={`${API_BASE}?op=file&id=${entry.id}`} target="_blank" className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                            <Download size={16} />
                          </a>
                          <a href={`${API_BASE}?op=file&id=${entry.id}&mode=view`} target="_blank" className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-xs underline">
                            Preview
                          </a>
                        </>
                      )}
