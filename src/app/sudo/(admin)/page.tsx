...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await fetch("/api/sudo/dashboard");
  ...
  const series = [
    { name: "Loads", data: data.monthlyLoads.map((m) => m.count) },
  ];
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
              ...
              <div className="flex items-end justify-between mt-5">
                ...
                <Badge color={data.stats.vendors.growth >= 0 ? "success" : "error"}>
                  {data.stats.vendors.growth >= 0 ? "↑" : "↓"}{" "}
                  {Math.abs(data.stats.vendors.growth)}%
                </Badge>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
              ...
              <div className="flex items-end justify-between mt-5">
                ...
                <Badge
                  color={data.stats.customers.growth >= 0 ? "success" : "error"}
                >
                  {data.stats.customers.growth >= 0 ? "↑" : "↓"}{" "}
                  {Math.abs(data.stats.customers.growth)}%
                </Badge>
              </div>
            </div>
          </div>
            ...
            <Table className="w-full">
              <TableHeader>
                <TableRow className="text-sm font-medium text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-800">
                  <TableCell isHeader className="py-3 text-left w-1/3">
                    Profile
                  </TableCell>
                  <TableCell isHeader className="py-3 text-left w-1/4">
                    Company
                  </TableCell>
                  <TableCell isHeader className="py-3 text-left w-1/4">
                    Phone
                  </TableCell>
                  <TableCell isHeader className="py-3 text-left w-1/6">
                    KYC Status
                  </TableCell>
                </TableRow>
              </TableHeader>
