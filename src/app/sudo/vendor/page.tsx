...
function VendorModal({
  ...
  async function save() {
    ...
    const r = await fetch("/api/sudo/vendor", { method, body: fd });
    if (!r.ok) {
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-5xl relative">
        ...
        <div className="p-5 border-b dark:border-gray-700 flex items-center justify-between">
          <h3 className="text-lg font-semibold">
            {isCreate ? "Create Vendor" : mode === "edit" ? "Edit Vendor" : "View Vendor"}
          </h3>
...
export default function VendorPage() {
  ...
  async function refresh() {
    const url = new URL(location.origin + "/api/sudo/vendor");
  ...
  useEffect(() => {
    (async () => {
      ...
      setMe(m);
      const ok = m?.role === "sudo" || (m?.permissions || []).includes("vendor:page_view");
      if (!ok) {
  ...
  async function doDelete() {
    ...
    const r = await fetch("/api/sudo/vendor", {
      method: "DELETE",
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <button
          onClick={openEdit}
          disabled={!can("vendor:edit") || selectedIds.length !== 1}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm ${
            can("vendor:edit") && selectedIds.length === 1
              ? "border-gray-300 text-gray-700 bg-white hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
              : "opacity-50 cursor-not-allowed border-gray-300 dark:border-gray-700"
          }`}
        >
        ...
        <button
          onClick={openView}
          disabled={!can("vendor:view") || selectedIds.length !== 1}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm ${
            can("vendor:view") && selectedIds.length === 1
              ? "border-gray-300 text-gray-700 bg-white hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
              : "opacity-50 cursor-not-allowed border-gray-300 dark:border-gray-700"
          }`}
        >
        ...
        <button
          onClick={doDelete}
          disabled={!can("vendor:delete") || selectedIds.length === 0}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm ${
            can("vendor:delete") && selectedIds.length > 0
              ? "border-gray-300 text-gray-700 bg-white hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
              : "opacity-50 cursor-not-allowed border-gray-300 dark:border-gray-700"
          }`}
        >
